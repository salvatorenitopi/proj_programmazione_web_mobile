...
function fx_run (){
	...
	if (name == "" || name == null){
		...
		$.ajax({
			url: 'http://127.0.0.1:8000/media_retweet?nome=' + name,
		})
...
function handle_data(data){
	...
	riscontro (data);
	add_crono ("Media Retweet", data.data.user.screen_name);

	var raw = data.data.raw_array; 
	var arr = [["Post", "Retweets"]];
	for (i=0; i<raw.length; i++){
		arr.push ([i + 1, raw[i]]);
	}

	var options = {
		title: "Numero di retweet per tweet",
