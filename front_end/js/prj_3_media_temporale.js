...
function handle_data(data){
	...
	riscontro (data);
	add_crono ("Media Temporale", data.data.user.screen_name);

	var raw = data.data.raw_array; 
	var arr = [["Post", "Tempo"]];
	for (i=0; i<raw.length; i++){
		arr.push ([(i+1).toString() + ":" + (i+2).toString(), raw[i]]);
	}

	var options = {
		title: "Medie fra coppie di Tweets",
		hAxis: {title: "Coppie Tweets",  titleTextStyle: {color: '#333'}},
		vAxis: {minValue: 0}
	};
